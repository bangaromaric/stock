...
/**
 * Performance test for the Inventaire entity.
 */
public class InventaireGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all inventaires").get("/api/inventaires").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new inventaire")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"quantite\": 0" +
                                ", \"inventaireDate\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_inventaire_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created inventaire").get("${new_inventaire_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created inventaire").delete("${new_inventaire_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Inventaire entity").exec(scn);
