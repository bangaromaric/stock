...
/**
 * Performance test for the Entrepot entity.
 */
public class EntrepotGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all entrepots").get("/api/entrepots").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entrepot")
                        .post("/api/entrepots")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entrepot_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created entrepot").get("${new_entrepot_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created entrepot").delete("${new_entrepot_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Entrepot entity").exec(scn);
