...
/**
 * Performance test for the PlansAbonnement entity.
 */
public class PlansAbonnementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all plansAbonnements").get("/api/plans-abonnements").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new plansAbonnement")
                        .post("/api/plans-abonnements")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"duree\": \"SAMPLE_TEXT\"" +
                                ", \"avantage\": null" +
                                ", \"deleteAt\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_plansAbonnement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created plansAbonnement").get("${new_plansAbonnement_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created plansAbonnement").delete("${new_plansAbonnement_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PlansAbonnement entity").exec(scn);
