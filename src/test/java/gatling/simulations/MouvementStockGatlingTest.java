...
/**
 * Performance test for the MouvementStock entity.
 */
public class MouvementStockGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mouvementStocks").get("/api/mouvement-stocks").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mouvementStock")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mouvementStock_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mouvementStock").get("${new_mouvementStock_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mouvementStock").delete("${new_mouvementStock_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MouvementStock entity").exec(scn);
