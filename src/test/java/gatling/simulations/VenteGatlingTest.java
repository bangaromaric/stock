...
/**
 * Performance test for the Vente entity.
 */
public class VenteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ventes").get("/api/ventes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new vente")
                        .post("/api/ventes")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"quantite\": 0" +
                                ", \"montant\": 0" +
                                ", \"venteDate\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_vente_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created vente").get("${new_vente_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created vente").delete("${new_vente_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Vente entity").exec(scn);
