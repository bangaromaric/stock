...
type RestOf<T extends IPermission | NewPermission> = Omit<T, 'deleteAt' | 'createdDate' | 'lastModifiedDate'> & {
...
export class PermissionService {
  ...
  getPermissionIdentifier(permission: Pick<IPermission, 'id'>): string {
  ...
  protected convertDateFromClient<T extends IPermission | NewPermission | PartialUpdatePermission>(permission: T): RestOf<T> {
    return {
      ...permission,
      deleteAt: permission.deleteAt?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restPermission: RestPermission): IPermission {
    return {
      ...restPermission,
      deleteAt: restPermission.deleteAt ? dayjs(restPermission.deleteAt) : undefined,
