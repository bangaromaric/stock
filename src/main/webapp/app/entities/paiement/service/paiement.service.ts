...
type RestOf<T extends IPaiement | NewPaiement> = Omit<T, 'deleteAt' | 'createdDate' | 'lastModifiedDate'> & {
...
export class PaiementService {
  ...
  getPaiementIdentifier(paiement: Pick<IPaiement, 'id'>): string {
    return paiement.id;
  }
  ...
  protected convertDateFromClient<T extends IPaiement | NewPaiement | PartialUpdatePaiement>(paiement: T): RestOf<T> {
    return {
      ...paiement,
      deleteAt: paiement.deleteAt?.toJSON() ?? null,
      createdDate: paiement.createdDate?.toJSON() ?? null,
      lastModifiedDate: paiement.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPaiement: RestPaiement): IPaiement {
    return {
      ...restPaiement,
      deleteAt: restPaiement.deleteAt ? dayjs(restPaiement.deleteAt) : undefined,
      createdDate: restPaiement.createdDate ? dayjs(restPaiement.createdDate) : undefined,
      lastModifiedDate: restPaiement.lastModifiedDate ? dayjs(restPaiement.lastModifiedDate) : undefined,
    };
  }
